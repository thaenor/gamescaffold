...
/**
 * Retrieves latest ticket update time from OTRS
 * @return int
 */
function getLastTimeFromTickets(){
    $query = "select change_time from ticket order by change_time desc limit 1";
    try{
        $result = pg_query($query) or die('Query failed: ' . pg_last_error());
        $data = (array_values(pg_fetch_all($result)));
        return intval($data[0]['change_time']);
    } catch(exception $e){
 ...
 * @return string
 */
function syncDBs($lastTime){

    $query = "DECLARE cursor1 CURSOR FOR select ti.id,
	ti.title,
	ti.user_id,
	/*us.first_name,
	us.last_name,*/
...
where ti.user_id=us.id AND ti.sla_id=sl.id AND ti.ticket_priority_id=tp.id AND ti.ticket_state_id=ts.id
	AND q.group_id = g.id AND ti.queue_id = q.id AND ti.type_id = type.id AND ti.change_time >= '".$lastTime."' order by ti.id";

    try {
        $dal = connect();
        $otrsTicketLastTime = getLastTimeFromTickets();
        if($lastTime > $otrsTicketLastTime){
            Log::warning('Nothing to sync. Stopping process.');
            return;
        }

        $con = new PDO("pgsql:host=10.200.10.54 port=5432 dbname=otrs user=otrsro password=otrs-ro123.");
        $con->beginTransaction(); // cursors require a transaction.
        $stmt = $con->prepare($query);
        $stmt->execute();
        $innerStatement = $con->prepare("FETCH 1000 FROM cursor1");
        ...
        var_dump($result);
        $data = (array_values(pg_fetch_all($result)));
        $chunkOfData = array_chunk($data, 200);
 ...
 * @return Exception
 */
function insertChunkToDB($element){
